...
# Usage   : DESKTOP_ENV={lxde|mate|xfce} [GROUP_ID=<gid>] fedora29-desktop.sh
# Depends : fedora29-container-base.sh, dnf.conf
# Creating image :
...
# Installing packages
BASE_PACKAGES="--exclude=elementary-greeter @base-x lightdm im-chooser @japanese-support langpacks-ja ibus-kkc net-tools xrdp"
...
cat << EOF > ${INSTALL_ROOT}/usr/local/bin/start-xrdp
#!/bin/sh
...
: end of file
EOF
chmod a+x ${INSTALL_ROOT}/usr/local/bin/start-xrdp

## Modifying xrdp.init max_bpp=32 => max_bpp=24
sed -e 's/^max_bpp=32/#max_bpp=32\
max_bpp=24/g' -i ${INSTALL_ROOT}/etc/xrdp/xrdp.ini
...
chmod +x ${INSTALL_ROOT}/etc/skel/.Xclients
...
chroot ${INSTALL_ROOT} useradd -G developers -p passwd developer
chroot ${INSTALL_ROOT} passwd -u developer
