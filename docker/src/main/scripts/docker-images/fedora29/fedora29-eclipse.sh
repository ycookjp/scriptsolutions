...
# Usage   : DESKTOP_ENV={lxde|mate|xfce} fedora29-eclipse.sh
# Depends : fedora29-desktop.sh, fedora29-container-base.sh, dnf.conf
# Creating image :
#   1. Start Fedora 29 and login as root.
...
ECLIPSE_URL=http://ftp.yz.yamagata-u.ac.jp/pub/eclipse/technology/epp/downloads/release/2018-12/R/eclipse-jee-2018-12-R-linux-gtk-x86_64.tar.gz
...
## Installing openjdk
dnf --installroot=${INSTALL_ROOT} install -y java-1.8.0-openjdk-devel webkit2gtk3
...
## Creating eclipse menu entry
  ...
  cat << EOF > ${INSTALL_ROOT}/usr/share/applications/eclipse.desktop
[Desktop Entry]
Version=1.0
Type=Application
Name=Eclipse
...
if [ _ != _${MIGU_URL} ]; then
  ...
  mkdir -p ${INSTALL_ROOT}/usr/share/fonts
  unzip -o -d ${INSTALL_ROOT}/usr/share/fonts ${DOWNLOAD_DIR}/`basename ${MIGU_URL}`
  rm -rf ${INSTALL_ROOT}/usr/share/fonts/migu-1m-20150712/ipag00303/ \
         ${INSTALL_ROOT}/usr/share/fonts/migu-1m-20150712/mplus-TESTFLIGHT-060/\
         ${INSTALL_ROOT}/usr/share/fonts/migu-1m-20150712/*.txt
fi
